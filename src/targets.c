...
int runStep(struct build_config *config, char *name) {
	...
	for(i = 0; i < config->steps_count; i++) {
		if(!strcmp(config->steps[i]->name, name)) {
			if(config->steps[i]->runIf != 0) {
				res = system(config->steps[i]->runIf);
				if(res) {
					printf("\e[1;32mTask \e[0;31m%s\e[1;32m does not need to be run.\e[0m\n", name);
					return 0;
				}
			}
			...
			if(config->steps[i]->dependencies != 0) {
			...
			if(config->steps[i]->commands != 0) for(j = 0; j < config->steps[i]->commands_count; j++) {
			...
			else printf("\e[1;32mTask \e[0;31m%s\e[1;32m is empty.\e[0m\n", name);
			puts("\e[1;31m------------\e[0m");
